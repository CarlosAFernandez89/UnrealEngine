...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TabListWidgetBase_Widget)

void UTabListWidgetBase_Widget::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void UTabListWidgetBase_Widget::NativeConstruct()
{
...
void UTabListWidgetBase_Widget::NativeDestruct()
{
	for (FSolviTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UTabListWidgetBase_Widget::GetPreregisteredTabInfo(const FName TabNameId, FSolviTabDescriptor& OutTabInfo)
{
	const FSolviTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FSolviTabDescriptor& TabInfo) -> bool
	{
...
void UTabListWidgetBase_Widget::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FSolviTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UTabListWidgetBase_Widget::RegisterDynamicTab(const FSolviTabDescriptor& TabDescriptor)
{
...
void UTabListWidgetBase_Widget::HandlePreLinkedSwitcherChanged()
{
	for (const FSolviTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UTabListWidgetBase_Widget::HandlePostLinkedSwitcherChanged()
{
...
void UTabListWidgetBase_Widget::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FSolviTabDescriptor* TabInfoPtr = nullptr;
	
	FSolviTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(USolviTabButtonInterface::StaticClass()))
	{
		...
		{
			ISolviTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool UTabListWidgetBase_Widget::IsFirstTabActive() const
{
...
bool UTabListWidgetBase_Widget::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool UTabListWidgetBase_Widget::IsTabVisible(FName TabId)
{
...
int32 UTabListWidgetBase_Widget::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UTabListWidgetBase_Widget::SetupTabs()
{
	for (FSolviTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
