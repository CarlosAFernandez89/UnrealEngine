...
void UMyAbilitySystemComponent::ReceiveDamage(UMyAbilitySystemComponent* SourceASC, float UnmitigatedDamage,
...
void UMyAbilitySystemComponent::ProcessAbilityInput(float DeltaTime, bool bGamePaused)
{
	...
	for (const FGameplayAbilitySpecHandle& SpecHandle : InputHeldSpecHandles)
	{
		if (const FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			...
			{
				const UMyGameplayAbility* SNAbility = CastChecked<UMyGameplayAbility>(AbilitySpec->Ability);

				if (SNAbility->GetActivationPolicy() == EMyAbilityActivationPolicy::WhileInputActive)
				{
		...
		if (FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			if (AbilitySpec->Ability)
			{
				...
				else
				{
					...
					if (SNAbility->GetActivationPolicy() == EMyAbilityActivationPolicy::OnInputTriggered)
					{
						AbilitiesToActivate.AddUnique(AbilitySpec->Handle);
					}
				}
