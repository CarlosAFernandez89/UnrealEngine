...
#include "MyAsyncTaskCooldownChanged.h"

UMyAsyncTaskCooldownChanged* UMyAsyncTaskCooldownChanged::ListenForCooldownChange(
	UAbilitySystemComponent* AbilitySystemComponent, FGameplayTagContainer InCooldownTags, bool InUseServerCooldown)
{
	UMyAsyncTaskCooldownChanged* ListenForCooldownChange = NewObject<UMyAsyncTaskCooldownChanged>();
	...
	AbilitySystemComponent->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(ListenForCooldownChange, &UMyAsyncTaskCooldownChanged::OnActiveGameplayEffectAddedCallback);
	...
	{
		AbilitySystemComponent->RegisterGameplayTagEvent(CooldownTag, EGameplayTagEventType::NewOrRemoved).AddUObject(ListenForCooldownChange, &UMyAsyncTaskCooldownChanged::CooldownTagChanged);
	}

	return ListenForCooldownChange;
}

void UMyAsyncTaskCooldownChanged::EndTask()
{
	if (IsValid(ASC))
	{
...
void UMyAsyncTaskCooldownChanged::OnActiveGameplayEffectAddedCallback(UAbilitySystemComponent* Target,
...
void UMyAsyncTaskCooldownChanged::CooldownTagChanged(const FGameplayTag CooldownTag, int32 NewCount)
{
	if (NewCount == 0)
	{
...
bool UMyAsyncTaskCooldownChanged::GetCooldownRemainingForTag(FGameplayTagContainer InCooldownTags, float& TimeRemaining,
