...
void ARPGPlayerControllerBase::Tick(float DeltaSeconds)
{
	Super::Tick(DeltaSeconds);
#if  !WITH_EDITOR
	CheckIfForeGroundWindow();
#endif
	
}

void ARPGPlayerControllerBase::CheckIfForeGroundWindow()
{
	ULocalPlayer* LocalPlayer = Cast<ULocalPlayer>(Player);
	if (!LocalPlayer->ViewportClient->Viewport || !LocalPlayer->ViewportClient->Viewport->IsForegroundWindow())
	{
		...
		MinimizeWindow();
		bDoOnGainFocusEvent = true;
	}
	else if(bDoOnGainFocusEvent)
	{
		URPGGameInstanceBase* RPGGameInstance = Cast<URPGGameInstanceBase>(UGameplayStatics::GetGameInstance(this));
...
void ARPGPlayerControllerBase::MinimizeWindow()
{
...
void UArenaCheatManager::ClearAllArenas()
{
	TArray<AActor*> ArenaManager;
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), AArenaManager::StaticClass(), ArenaManager);

	const AArenaManager* WorldArenaManager = Cast<AArenaManager>(ArenaManager[0]);

	if(WorldArenaManager)
	{
		for (const auto Arena : WorldArenaManager->Arenas)
		{
			Arena->GetWorldTimerManager().ClearAllTimersForObject(WorldArenaManager->CurrentArena);
			Arena->OnArenaCleared();

			TArray<AActor*>EnemiesInArena = Arena->GetAllEnemiesInArena();
			for (const auto Enemy : EnemiesInArena)
			{
				ARPGCharacterBase* En = Cast<ARPGCharacterBase>(Enemy);
				if(En)
				{
					if(!En->bIsPlayer)
					{
...
void UArenaCheatManager::ClearArena()
{
	TArray<AActor*> ArenaManager;
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), AArenaManager::StaticClass(), ArenaManager);
	...
	if(WorldArenaManager)
	{
		if(WorldArenaManager->CurrentArena != nullptr)
		{

			WorldArenaManager->CurrentArena->GetWorldTimerManager().ClearAllTimersForObject(WorldArenaManager->CurrentArena);
			WorldArenaManager->CurrentArena->OnArenaCleared();
			TArray<AActor*>EnemiesInArena = WorldArenaManager->CurrentArena->GetAllEnemiesInArena();
			for (AActor* Enemy : EnemiesInArena)
			{
				ARPGCharacterBase* En = Cast<ARPGCharacterBase>(Enemy);
				if(En)
				{
