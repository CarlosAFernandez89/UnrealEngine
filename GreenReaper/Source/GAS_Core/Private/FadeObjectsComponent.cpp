...
void UFadeObjectsComponent::BeginPlay()
{
	...
	GetOwner()->GetWorld()->GetTimerManager().SetTimer(timerHandle_AddObjectsTimer, this, &UFadeObjectsComponent::AddObjectsToHideTimer, 0.0166, true);
	GetOwner()->GetWorld()->GetTimerManager().SetTimer(timerHandle_ObjectComputeTimer, this, &UFadeObjectsComponent::FadeObjWorkerTimer, 0.0166, true);
...
void UFadeObjectsComponent::AddObjectsToHideTimer()
{
    ...
    for (AActor* currentActor_ : characterArray)
    {
        ...
        const FVector traceEnd_ = currentActor_->GetActorLocation() + FVector(0.f,0.f, 88.f);
        ...
        if (traceLength_.Size() < workDistance)
        {
        	...
        	//DrawDebugLine(GetWorld(), traceStart_, traceEnd_,FColor::Cyan);

            for (int hA = 0; hA < hitArray_.Num(); ++hA)
            {
	            if(hitArray_[hA].GetActor()->ActorHasTag("No Occlusion"))
	            {
	            	hitArray_.RemoveAt(hA);
	            }
            }
